  ...
  const loadFeedback = async () => {
    try {
      ...
      if (data.user_feedback) {
        setForm({
          rate: data.user_feedback.RATE,
          ftext: data.user_feedback.FTEXT,
        });
      }
    } catch (err) {
            ...
            <form onSubmit={handleSubmit} className="section">
              <div className="form-row">
                ...
                <textarea
                  ...
                  value={form.ftext}
                  onChange={(e) =>
                    setForm((prev) => ({ ...prev, ftext: e.target.value }))
                  }
              ...
              <table className="table">
                ...
                <tbody>
                  {detail.episodes.map((e) => (
                    <tr key={e.EID}>
                      <td>Episode {e.E_NUM}</td>
                      <td>
                        Start: {e.SCHEDULE_SDATE}
                        <br />
                       End: {e.SCHEDULE_EDATE}
                      </td>
                      <td>{e.NVIEWERS}</td>
          ...
          <div className="section card">
            ...
            {feedbackData ? (
              <>
                <p className="muted">
                  Avg Rating:{" "}
                  {feedbackData.stats?.avg_rating
                    ? parseFloat(feedbackData.stats.avg_rating).toFixed(2)
                    : "—"}{" "}
                ...
                {feedbackData.feedback_list?.length ? (
                  <ul>
                    {feedbackData.feedback_list.map((f, idx) => (
                      <li key={idx}>
                        <strong>{f.USERNAME}</strong> rated {f.RATE} on{" "}
                        {new Date(f.FDATE).toISOString().slice(0, 10)}
                        <br />
